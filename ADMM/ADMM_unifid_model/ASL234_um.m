function [V_1,V_2,iter,obj_rec,difY1_rec,difY2_rec] = ASL234_um(L1,L2,L3,cls_num,opts,lambda1,lambda2)
...
%       L2      -       n^2*n*2 LapMatrix with pairwise similarity
%       L3      -       n^2*n Lapmatrix with tri similarity
...
%         end
        ...
        %Update V1
        V1_k = update_V1_L234(V1_k,V2_k,L1,L3,Y1_k,Y2_k,mu,alpha,0);
...
%         end
        ...
        A = (mu*eye(n_2)-2*L2);
        B = (L3*V1_k+Y1_k+mu*(kron_col(V1_k)));
        for B_n = 1:size(B,2)
            V2_k(:,B_n) = gmres(A,B(:,B_n));
        end
%        V2_k = inv(mu*eye(n_2)-2*L2)*(L3*V1_k+Y1_k+mu*(kron_col(V1_k)));
%        V2_k = gmres((mu*eye(n_2)-2*L2),(L3*V1_k+Y1_k+mu*(kron_col(V1_k))));
    ...
    if DEBUG
        if iter==1 || mod(iter, 10)==0
            obj = obj_f(V1_k,V2_k,L1,L2,L3,mu,Y1_k,Y2_k);
            disp(['iter ' num2str(iter) ', obj=' num2str(obj) ', mu=' num2str(mu)]);                  
        end
    end
%     LL1 = -trace(V1_k'*L1*V1_k);
%     LL2 = -trace(V2_k'*L2*V2_k);
%     LL3 = -trace(V2_k'*L3*(V1_k.*V1_k));
    
    obj_rec(iter,:) = obj_f(V1_k,V2_k,L1,L2,L3,mu,Y1_k,Y2_k);
...
function Z = obj_f(V1,V2,L1,L2,L3,mu,Y1,Y2)
    [m1,n1] = size(V1);
    [m2,n2] = size(V2);
    z1 = trace(V1'*L1*V1);
    z2 = trace(V2'*L2*V2);
