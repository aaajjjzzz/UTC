function [ACC, ARI, F_SCORE, NMI, Purity] = ClusterMeasure(truth, label)
ACC= accuracyMeasure(truth, label);
ARI = adjrand(truth, label);
[F_SCORE,~,~] = fprMeasure(truth, label);
NMI = nmi(truth, label);
Purity = purityMeasure( truth, label);
