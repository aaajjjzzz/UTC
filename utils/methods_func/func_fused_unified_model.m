function finale_result = func_fused_unified_model(data,gt,dataset_name,save_path, order4_sim_opt)
...
%  
    ...
    %% undecom
    %2+3 order
    lambda = [0.1,0.5, 1, 1.5]; cell_it= 1; embedding_bank={};results_bank = {};
    lambda_2 = [0.05, 0.1, 0.25, 0.5, 0.75,1,1.5];
    %lambda_2 = [0.1, 0.5, 0.75,1,1.5];
    l2_size = size(lambda_2,2);
    l1_size = size(lambda,2);
    for loop_1 = 2:l1_size
        for loop_2 = 1:l2_size
            lambda1 =lambda(loop_1);
            lambda2 =lambda_2(loop_2);
            opts.tol = 1e-3;
            opts.max_iter = 100;
            opts.rho = 1.1;
            opts.mu = 1e0;
...
%             [V_1_ADMM_L23,V_2_ADMM_L23,iter_L23,obj_rec_ADMM_L23,difY1_rec1_L23,difY2_rec1_L23] = ASL23_um(L_2,L_3,cls_num,opts,lambda2);
...
%             [V_1_ADMM_L23_gc,V_2_ADMM_L23_gc,iter_L23_gc,obj_rec_ADMM_L23_gc,difY1_rec1_L23_gc,difY2_rec1_L23_gc] = ASL23_um(L_2,L_3_gc,cls_num,opts,lambda2);
...
%             [V_1_ADMM_L24,V_2_ADMM_L24,iter_L24,obj_rec_ADMM_L24,difY1_rec1_L24,difY2_rec1_L24] = ASL24_um(L_2,L_4,cls_num,opts,lambda1);
%             [~,group_L24_SC,~] = SpectralClustering5(V_1_ADMM_L24, cls_num);
%             [ACC(it), ARI(it), F_SCORE(it), NMI(it), Purity(it)] = ClusterMeasure(gt,group_L24dataset_name_SC);
%             it = it+1;
            %2+3+4 order
            Method_Name(it) = "ADMM_undecom_L234_lambda1="+string(lambda1)+"_lambda2="+string(lambda2);
            [V_1_ADMM_L234,V_2_ADMM_L234,iter_L234,obj_rec_ADMM_L234,difY1_rec1_L234,difY2_rec1_L234] = ASL234_um(L_2,L_4,L_3,cls_num,opts,lambda1,lambda2);
...
%             [V_1_ADMM_L234_gc,V_2_ADMM_L234_gc,iter_L234_gc,obj_rec_ADMM_L234_gc,difY1_rec1_L234_gc,difY2_rec1_L234_gc] = ASL234_um(L_2,L_4,L_3_gc,cls_num,opts,lambda1,lambda2);
